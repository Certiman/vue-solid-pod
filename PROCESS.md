...
- `dul:realizes`: **[NEW]** the resource name where the form data should be stored (e.g., "org", "site", "unit"). This property uses the DUL (DOLCE+DnS Ultralite) ontology and is the preferred approach for defining storage targets.
...
**Preferred Approach (dul:realizes)**: Each step should define a `dul:realizes` property that specifies the resource name for data storage:
...
**Legacy Fallback**: If `dul:realizes` is not defined, the system will derive the resource name from the task name using intelligent mapping (e.g., "addSite" → "site", "addUnit" → "unit").
